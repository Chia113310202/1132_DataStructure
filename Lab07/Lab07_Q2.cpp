...
public:
    ...
    int findTreeMax(TreeNode* node){
    	...
    	int max = node->value; // 先假設最大值是 node->value 
    	
    	queue<TreeNode*> q;
    	q.push(node);
    	
    	while (!q.empty() ) {
            TreeNode* current = q.front(); // 取出 queue中的節點
            q.pop();
			
			if (current->value > max) { // 如果這個節點的值比 max還大就更新 max 
            	max = current->value;
        	}

        	if (current->left) { // 因為一次只能檢查一個，所以如果有左子節點就加進 queue裡 
            	q.push(current->left);
        	}

        	if (current->right) { // 如果有右子節點就加進 queue裡
...
int main() {
    ...
    cout << "左子樹最大值 : " << tree.findTreeMax(tree.root->left) << endl;
    cout << "右子樹最大值 : " << tree.findTreeMax(tree.root->right) << endl;
